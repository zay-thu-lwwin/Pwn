...
Stack size က program run နေစဉ်လိုအပ်သလို dynamic ပြောင်းနိုင်တယ်။ Kernel က လက်ရှိသုံးထားတဲ့အပိုင်းကိုပဲ ပြထားတာ။ Limit ရောက်မှ segmentation fault ဖြစ်မယ်
info proc all နဲ့ ကြည့်နိုင်
...
```
Intel/NASM Format: CMD <destination>, <source> <;comments>
...
break *main+59
info break (i b)
delete break 1 (del b)
d 5 (delete break no 5)
run
...
Memory exceptions ဆိုတာ operating system က software တစ်ခုခုမှာ အမှားတွေ့ရင် ဒါမှမဟုတ် execute လုပ်နေစဉ်မှာ ပြဿနာတက်ရင် တုံ့ပြန်တဲ့ သဘောပါ။ လွန်ခဲ့တဲ့ ဆယ်စုနှစ်အတွင်း program flow တွေမှာ ဖြစ်တဲ့ security vulnerability အများစုရဲ့ အကြောင်းရင်းက ဒါပါပဲ။

C ဒါမှမဟုတ် C++ လိုမျိုး low-level language တွေနဲ့ ပရိုဂရမ်ရေးတဲ့အခါ သတိမထားမိတဲ့အတွက် programming error တွေ မကြာခဏဖြစ်တယ်။ ဒီ error တွေက buffer overflow ကို ဖြစ်စေတယ်။

ဒီ language တွေက machine code ကို တိုက်ရိုက်လိုလို compile လုပ်တယ်။ Java ဒါမှမဟုတ် Python လိုမျိုး high-level language တွေနဲ့ မတူဘဲ operating system ရဲ့ control structure ကို အနည်းငယ်ပဲ ဖြတ်သန်းတယ်။

Buffer overflow ဆိုတာ operating system ရဲ့ memory ထဲက buffer တစ်ခုထဲကို သွင်းလို့ရတဲ့ပမာဏထက် ပိုကြီးတဲ့ data ကို ထည့်လိုက်တဲ့အခါ ဖြစ်တဲ့ error ပါ။ ဒီလို မှားယွင်းစွာ ကိုင်တွယ်မိတဲ့အတွက် program ရဲ့ တခြား function တွေရဲ့ memory ကို ပြန်ရေးမိသွားပြီး security vulnerability ဖြစ်လာနိုင်တယ်။

ဒီလို program (binary file) ဆိုတာ data storage medium ပေါ်မှာ သိမ်းဆည်းထားတဲ့ executable file တစ်ခုပါ။ ဒီလို executable binary file တွေအတွက် ပုံစံအမျိုးမျိုးရှိတယ်။ ဥပမာ Microsoft platform တွေမှာ Portable Executable Format (PE) ကို သုံးတယ်။

နောက်တစ်မျိုးက Executable and Linking Format (ELF) ဖြစ်တယ်။ ဒါကို ခေတ်ပေါ် UNIX variant အားလုံးနီးပါးက support လုပ်တယ်။ Linker က ဒီလို executable binary file ကို load လုပ်ပြီး program ကို execute လုပ်တဲ့အခါ program code ကို main memory ထဲကို ထည့်ပြီး CPU က execute လုပ်တယ်။

Program တွေက data နဲ့ instruction တွေကို initialization လုပ်စဉ်နဲ့ execution လုပ်စဉ်မှာ memory ထဲမှာ သိမ်းတယ်။ ဒါတွေက execute လုပ်နေတဲ့ software ထဲမှာ ပြသထားတဲ့ data (သို့) user က ထည့်လိုက်တဲ့ data တွေပါ။ အထူးသဖြင့် user input အတွက် buffer တစ်ခုကို ကြိုတင်ဖန်တီးထားရတယ်။

Instruction တွေက program flow ကို ပုံဖော်ဖို့ သုံးတယ်။ အဲဒီအထဲမှာ return address တွေကို memory ထဲမှာ သိမ်းထားတယ်။ ဒီ return address တွေက တခြား memory address တွေကို ညွှန်ပြပြီး program ရဲ့ control flow ကို သတ်မှတ်ပေးတယ်။

ဒီလို return address ကို buffer overflow သုံးပြီး တမင်တကာ ပြန်ရေးလိုက်ရင် attacker က program flow ကို ခြယ်လှယ်နိုင်တယ်။ ဥပမာ return address က တခြား function (သို့) subroutine ကို ညွှန်ပြအောင် လုပ်တာမျိုးပေါ့။ ဒါ့အပြင် user input ကနေ ကြိုထည့်ထားတဲ့ code ဆီကို ပြန်ခုန်သွားအောင်လည်း လုပ်နိုင်တယ်



### Prologue (အဝင်)

Function တစ်ခုစတဲ့အခါ လုပ်လေ့ရှိတဲ့ အဆင့် ၃ ဆင့်ကို **Prologue** လို့ခေါ်တယ်။

1. `push ebp` - ရှေ့က function ရဲ့ EBP တန်ဖိုးကို stack ပေါ် သိမ်းတယ်
    
2. `mov ebp, esp` - Stack frame အသစ် စတင်ဖို့ EBP ကို ESP ရဲ့ တန်ဖိုးထားလိုက်တယ်
    
3. `sub esp, 0x404` - local variable တွေနဲ့ operations အတွက် နေရာ ဖန်တီးတယ် (ESP ကို အောက်ကိုရွှေ့တယ်)

### Epilogue (အထွက်)

Function ပြီးသွားရင် ပြန်ထွက်တဲ့အခါ **Epilogue** လုပ်တယ်။ ESP ကို EBP နဲ့ အစားထိုးပြီး မူလတန်ဖိုးကို ပြန်ရယူတယ်။

bash

(gdb) disas bowfunc 
   0x00000580 <+51>:    leave   # ESP ကို EBP နဲ့ အစားထိုး၊ ရှေ့က EBP ကို ပြန်ယူ
   0x00000581 <+52>:    ret      # stack ပေါ်က return address ကို ခုန်သွားတယ်

`leave` instruction က `mov esp, ebp` နဲ့ `pop ebp` ကို တစ်ခါတည်းလုပ်တယ်။  
`ret` instruction က stack ပေါ်က return address ကို EIP ထဲထည့်ပြီး အဲဒီနေရာကို ခုန်သွားတယ်။