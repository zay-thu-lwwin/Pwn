...
3. **Architecture/Platform Differences**  
    သင့်မှာရှိတဲ့ libc နှစ်ခုကို မတူတဲ့ system ကနေ download လုပ်ထားတာ (သို့) compile လုပ်ထားတာ ဖြစ်နိုင်တယ်။
...
│ 2. Reads ELF header                                     │
│ 3. Sees INTERP = "/lib64/ld-linux-x86-64.so.2"         │
│ 4. Loads dynamic linker (ld.so)                        │
└──────────────┬────────────────────────────────────────────┘
               │
               ▼
┌─────────────────────────────────────────────────────────┐
│ Dynamic Linker (ld.so) starts                          │
...
┌─────────────────────────────────────────────────────────┐
│ libc.so.6 loaded                                       │
│   - Provides printf, malloc, etc.                      │
│   - Initializes malloc states                          │
│   - Sets up hooks                                      │
└─────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Your Binary   │    │     ld.so       │    │    libc.so      │
│   (hello)       │    │  (ld-2.23.so)   │    │ (libc-2.23.so)  │
├─────────────────┤    ├─────────────────┤    ├─────────────────┤
│ INTERP header   │───▶│ _dl_start()     │───▶│ __libc_start_   │
│   points to     │    │   Loads libc    │    │   main()        │
│   ld.so         │    │   Resolves      │    │   malloc_init() │
│                 │    │   symbols       │    │   Sets up       │
│ PLT/GOT tables  │◀──▶│ Updates GOT     │◀──▶│   main_arena    │
│                 │    │                 │    │   __malloc_hook │
│ call malloc@plt │───▶│ _dl_resolve()   │───▶│ malloc()        │
└─────────────────┘    └─────────────────┘    └─────────────────┘
```
...
![](pwn_repo/Basics/pictures/zero-4.jpg)
pwn_repo/Basics/pictures/0ctf-4.png
pwn_repo/Basics/pictures/zero-4.jpg
