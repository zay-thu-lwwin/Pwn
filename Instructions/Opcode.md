...
CPU က assembly mnemonic ကို နားမလည်ဘူး
...
add al, 0x01          →        00000100 00000001  (04 01)
mov eax, 0x12345678   →        10111000 01111000 01010110 00110100 00010010 (B8 78 56 34 12)
```



x86 instruction တစ်ခုက  အပိုင်း (6) ပိုင်း ပါဝင်နိုင်တယ်
```
...
64-bit Instruction Format (REX prefix optional):
┌───────┬─────────┬─────────┬──────────┬──────────┬──────────┐
...
imm8 = immediate 8-bit (8-bit တန်ဖိုး)
ဆိုလိုတာက တိုက်ရိုက်ထည့်လိုက်တဲ့ 8-bit ဂဏန်း (0 ကနေ 255 အထိ)
...
|---|---|---|
|**Bit**|0 သို့ 1|1 bit|
|**Nibble**|4 bits တွဲတာ|4 bits|
|**Byte**|8 bits (Nibble နှစ်ခု)|8 bits|

**ဥပမာ - B0 (1011 0000)**

- High Nibble = `1011` (ဘယ်ဘက် 4 bits)
    
- Low Nibble = `0000` (ညာဘက် 4 bits)

```
┌─────────────────────────────────────────────────────────────────────┐
│                                                                     │
│   High Nibble (1011) → "ဘယ် instruction မျိုးလဲ" ကို ပြတယ်        │
│                      → MOV, ADD, SUB, JMP, CMP, စသည်ဖြင့်          │
│                                                                     │
│   Low Nibble (0000)  → "ဘယ် register နဲ့လဲ" ကို ပြတယ်             │
│                      → AL, CL, DL, BL, AH, CH, DH, BH စသည်ဖြင့်   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
|---|---|---|---|
|**B0**|1011 (MOV group)|0000 (0)|**AL**|
|**B1**|1011 (MOV group)|0001 (1)|**CL**|
|**B2**|1011 (MOV group)|0010 (2)|**DL**|
|**B3**|1011 (MOV group)|0011 (3)|**BL**|
|**B4**|1011 (MOV group)|0100 (4)|**AH**|
|**B5**|1011 (MOV group)|0101 (5)|**CH**|
...
| ------ | ---------------------- | ---------- | ------ | ------ | ---------------- |
| **B8** | 1011 (MOV group)       | 1000 (8)   | AX     | EAX    | RAX              |
| **B9** | 1011 (MOV group)       | 1001 (9)   | CX     | ECX    | RCX              |
| **BA** | 1011 (MOV group)       | 1010 (10)  | DX     | EDX    | RDX              |
| **BB** | 1011 (MOV group)       | 1011 (11)  | BX     | EBX    | RBX              |
| **BC** | 1011 (MOV group)       | 1100 (12)  | SP     | ESP    | RSP              |
| **BD** | 1011 (MOV group)       | 1101 (13)  | BP     | EBP    | RBP              |
| **BE** | 1011 (MOV group)       | 1110 (14)  | SI     | ESI    | RSI              |
| **BF** | 1011 (MOV group)       | 1111 (15)  | DI     | EDI    | RDI              |



# - Low Nibble (Register Number) ဇယားများ

## 8.1 8-bit Register Number (Low Nibble 0-7)

|Low Nibble|Hex|8-bit Register|
|---|---|---|
|0000|0|AL|
|0001|1|CL|
|0010|2|DL|
...
## 8.2 16/32/64-bit Register Number (Low Nibble 8-F)

| Low Nibble | Hex | 16-bit | 32-bit | 64-bit |
| ---------- | --- | ------ | ------ | ------ |
| 1000       | 8   | AX     | EAX    | RAX    |
| 1001       | 9   | CX     | ECX    | RCX    |
| 1010       | A   | DX     | EDX    | RDX    |
